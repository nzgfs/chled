...
{
		{ GPX1_DAT_0_POS,GPX2_DAT_7_POS},
		{ GPX1_DAT_0_MSK,GPX2_DAT_7_MSK},
		{ GPX1_DAT_0_CLR,GPX2_DAT_7_CLR}
};
...
static int FS4412_LED_ioremap(void)
{
	FS4412_LED.reg[0] = ioremap(GPX1_BASE, 8);
	if (!FS4412_LED.reg[0])
		return -EIO;
	FS4412_LED.reg[1] = ioremap(GPX2_BASE, 8);
	if (!FS4412_LED.reg[1])
		return -EIO;
	return 0;
}
...
static void FS4412_LED_init(void)
{
	writel(readl(&FS4412_LED.reg[0]->CON) &  GPX1_CON_0_CLR| GPX1_CON_0_OUT, 
			&FS4412_LED.reg[0]->CON);
	writel(readl(&FS4412_LED.reg[1]->CON) & GPX2_CON_7_CLR| GPX2_CON_7_OUT,
			&FS4412_LED.reg[1]->CON);
	FS4412_LED_deinit();
}
...
static void FS4412_LED_deinit(void)
{
	...
	writel(FS4412_LED.dat[0] & FS4412_LED_reg_ops.GPIO_DAT_CLR[0],
			&FS4412_LED.reg[0]->DAT);
	writel(FS4412_LED.dat[1] & FS4412_LED_reg_ops.GPIO_DAT_CLR[1],
			&FS4412_LED.reg[1]->DAT);
...
static int FS4412_LED_rotate(void)
{
	...
	do
	{
		...
		n =  2;
		if(dir){pos[0]=1;pos[1]=0;}
		else{pos[0]=0;pos[1]=1;}
		FS4412_LED_state(LOCK);
		for (i = 0; i < n; i++)
		{
